...
const Crush= () => {
  ...
  const checkForColumnOfFour = () => {
    for(let i=0;i<=39;i++){
      ...
      if( columnOfFour.every(square => currentArrangment[square
  ...
  const checkForRowOfFour = () => {
    for(let i=0;i<64;i++){
      ...
      const decidedColor = currentArrangment[i];
      const isBlank = currentArrangment[i] === blank;
      const notValid = [5,6,7,13,14,15,21,22,23,29,30,31,37,38,39,45,46,47,54,55,62,63,64];
      if(notValid.includes(i))continue;
      if( rowOfFour.every(square => currentArrangment[square
      ]=== decidedColor && !isBlank)){
        if(gameStarted){
          setScore((scores)=> scores+4);
        }
  ...
  const checkForColumnOfThree = () => {
    for(let i=0;i<=47;i++){
      ...
      const decidedColor = currentArrangment[i];
      const isBlank = currentArrangment[i] === blank;
      if( columnOfThree.every(square => currentArrangment[square
      ]=== decidedColor&& !isBlank)){
        if(gameStarted){
  ...
  const checkForRowOfThree = () => {
    for(let i=0;i<64;i++){
      ...
      if(notValid.includes(i))continue;
      if( rowOfThree.every(square => currentArrangment[square
      ]=== decidedColor&& !isBlank)){
        if(gameStarted){
          setScore((scores)=> scores+3);
        }
  ...
  const dragEnd = (e) => {
    ...
    currentArrangment[squareBeingDraggedId]= squareBeingReplaced.getAttribute('src');
    currentArrangment[squareBeingReplacedId]=squareBeingDragged.getAttribute('src');
    ...
    if(squareBeingReplacedId && validMove && (C4 || R4|| C3 || R3)){
      ...
      currentArrangment[squareBeingDraggedId]= squareBeingDragged.getAttribute('src');
      currentArrangment[squareBeingReplacedId]= squareBeingReplaced.getAttribute('src');
    }
  ...
  },[checkForColumnOfFour,checkForRowOfFour,checkForColumnOfThree,checkForRowOfThree,moveIntoSquareBelow, currentArrangment])
  ...
  return (
    <div className="crush">
      ...
      <Score score={score} passingScore={passingScore} numberofAction={numberofAction}/>
      {losemodalOpen && <Modal title={"You Lose!!" } titlecolor="red" score={score} passingScore={passingScore} 
      BTM={BTM} PA={PA} />}
      {winmodalOpen && <Modal title={"You Win!!" } titlecolor="green" score={score} passingScore={passingScore} 
      BTM={BTM} PA={PA} />}
      {menuOpen && <Menu  setMenuOpen={setMenuOpen} StartNewGame={StartNewGame} setHTPOpen={setHTPOPen}/>}
      {HTPOpen && <HTP  BTM={BTM}/>}
    </div>
  );
}
